...
export default class ScheduleHome extends Component {
  ...
  _updateAlarm = async () => {
    const { selectedTask } = this.state;
    eventId = selectedTask.alarm.createEventAsyncRes.toString();
    const event = {
      title: selectedTask.title,
      notes: selectedTask.notes, //"For " + selectedTask.patient + ": " + selectedTask.notes,
      ...
      alarms: selectedTask.alarm.isOn ? [
        {
          relativeOffset: -5,
          method: 'alert'
        }
    ...
    const details = {
      email: selectedTask.patient,
      name: selectedTask.patient,
    ...
    if (selectedTask.alarm.createEventAsyncRes === '') {
      try {
        const createEventAsyncRes = await Calendar.updateEventAsync(
          eventId,
          event
        );

        const updateAttendeeToEventRes = await Calendar.updateAttendeeAsync(
          selectedTask.alarm.addAttendeeAsyncRes.toString(),
          details
        );

        const updateTask = { ...selectedTask };
        updateTask.alarm.createEventAsyncRes = createEventAsyncRes;
        updateTask.alarm.addAttendeeAsyncRes = updateAttendeeToEventRes;
      ...
      try {
        Calendar.updateEventAsync(
          ...
          event
        ).then(Calendar.deleteAttendeeAsync(selectedTask.alarm.addAttendeeAsyncRes.toString())
        .then(Calendar.createAttendeeAsync(
          selectedTask.alarm.createEventAsyncRes.toString(),
          details
        ))
  ...
  _deleteAlarm = async () => {
    ...
    try {
      Calendar.deleteAttendeeAsync(selectedTask.alarm.addAttendeeAsyncRes.toString()).then(
        Calendar.deleteEventAsync(selectedTask.alarm.createEventAsyncRes)
      )      
          ...
          <>
            {selectedTask !== null && (
              ...
              <Task isModalVisible={isModalVisible}>
                ...
                <Icon style={styles.close} name="cancel" type="MaterialIcons"
                  onPress={
                    () => this.setState({
                      isModalVisible: false
                    })
                  ...
                  <TextInput
                    ...
                    placeholder="What do you need your patient to do?"
                  />
                  <Text
                    style={{
                      fontSize: 14,
                  ...
                  <View style={{ flexDirection: 'row' }}>
                      <TouchableOpacity style={styles.checkup}
                        onPress={() => {
                          const prevSelectedTask = { ...selectedTask };
                          prevSelectedTask.title = "Go for check-up";
                          this.setState({
                            selectedTask: prevSelectedTask,
                          });
                        }}>
                      ...
                      <TouchableOpacity style={styles.eatMeds}
                        onPress={() => {
                          const prevSelectedTask = { ...selectedTask };
                          prevSelectedTask.title = "Have medicines";
                          this.setState({
                            selectedTask: prevSelectedTask,
                          });
                        }}>
                    ...
                    <TouchableOpacity style={styles.sleep}
                        onPress={() => {
                          const prevSelectedTask = { ...selectedTask };
                          prevSelectedTask.title = "Go to sleep";
                          this.setState({
                            selectedTask: prevSelectedTask,
                          });
                        }}>
                    ...
                    <AuthContext.Consumer>
                      {
                        authContext => (
                          <Picker
                            ...
                            selectedValue={selectedTask.patient}
                            onValueChange={(itemValue, itemIndex) => {
                              const prevSelectedTask = { ...selectedTask };
                              prevSelectedTask.patient = itemValue;
                              this.setState({
                                selectedTask: prevSelectedTask,
                              })}}
                          >
            ...
            >
              <HeaderComponent headerTitle="Schedules" notifCount="5" showLogo={false}/>
              <View style={{ flex: 1 }}>
                <CalendarStrip
                  ref={ref => {
                ...
                />
                {
                  todoList.some(item => `${moment(item.alarm.time.toString()).format('YYYY')}-${moment(item.alarm.time.toString()).format('MM')}-${moment(item.alarm.time.toString()).format('DD')}` == currentDate.toString()) ?
                  <Text></Text> 
                  : 
                  <>
                  <Text style={{
                      ...
                      >
                        ...
                        <View>
                          ...
                          >
                            ...
                            >{`${moment(item.alarm.time).format('LT')}`}  {item.patient}
                            {"\n"}
                            {item.notes.split(":")[1]}                            
                            </Text>
